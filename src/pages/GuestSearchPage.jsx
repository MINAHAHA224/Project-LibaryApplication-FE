...
import React, { useState, useEffect, useCallback } from 'react';
import { Table, Spin, Input, Typography, Image, Tag, App, Button } from 'antd';
import { LoginOutlined } from '@ant-design/icons';
import { useNavigate } from 'react-router-dom';
import { getPublicBookTitles } from '../services/publicService';
import { formatDate } from '../utils/helpers';
// Không cần import BookSubTable nữa
import './GuestSearchPage.css';
...
const GuestSearchPage = () => {
    ...
    const handleSearch = (value) => {
        ...
        const filtered = bookTitles.filter(item =>
            (item.codeBookTitle && item.codeBookTitle.toLowerCase().includes(lowercasedValue)) ||
            (item.nameBook && item.nameBook.toLowerCase().includes(lowercasedValue)) ||
            (item.nameAuthor && item.nameAuthor.toLowerCase().includes(lowercasedValue)) ||
            (item.nameCodeType && item.nameCodeType.toLowerCase().includes(lowercasedValue))
        );
        setFilteredBookTitles(filtered);
    };

    // === BỎ CỘT THAO TÁC VÀ THÊM CÁC CỘT KHÁC ===
    const columns = [
        {
            title: 'Ảnh bìa', dataIndex: 'picturePath', key: 'picturePath', width: 100,
            render: (path) => <Image width={80} src={`http://localhost:8080/dausach/${path}`} fallback="/images/fallback.png" />
